...
public class video1 extends Thread{
	boolean bool=true;
	SessionAddress local;
	SessionAddress target;

    public video1(InetAddress target) throws UnknownHostException {
        local = new SessionAddress(InetAddress.getLocalHost(),50000);
    	this.target=new SessionAddress(target,60000);
    	this.start();
    }
     public void run(){
